...
- Identify environment (dev/prod/test)
...
| Race condition | Async operations, state updates |
| Off-by-one | Array bounds, loop conditions |
| Type mismatch | API response shape, schema drift |
| State leak | useEffect cleanup, subscriptions |
| Cache stale | Query invalidation, refetch triggers |
