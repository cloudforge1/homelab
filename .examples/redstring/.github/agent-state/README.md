...
When multiple agent sessions work concurrently (e.g., `@impl-expo` on UI while `@impl-supabase` on backend), they need coordination to:
...
**Examples**:
- `impl-expo-lx5q8zk3-a7b2`
- `orchestrator-lx5qa2m1-c9d4`
- `impl-supabase-lx5qb7n2-e5f6`

**Components**:
| Part | Description |
|------|-------------|
| `agentName` | Agent identifier (e.g., `impl-expo`, `orchestrator`) |
...
interface Session {
  ...
  agent: string;                 // Agent name (e.g., "impl-expo")
...
```json
{
  ...
  "sessions": [
    {
      "id": "impl-expo-lx5q8zk3-a7b2",
      "agent": "impl-expo",
      "status": "active",
      "startedAt": "2026-01-15T10:30:00Z",
      "expiresAt": "2026-01-15T11:00:00Z",
      "workingOn": "Implementing PresenceDot animation",
      "lockedFiles": ["src/components/PresenceDot.tsx"]
    }
  ]
}
```
...
**Example**: 
- Path: `src/components/PresenceDot.tsx`
...
```json
{
  "sessionId": "impl-expo-lx5q8zk3-a7b2",
  "filePath": "src/components/PresenceDot.tsx",
  "lockedAt": "2026-01-15T10:30:00Z",
  "expiresAt": "2026-01-15T11:00:00Z",
  "reason": "Implementing spring animation for presence indicator"
}
```
...
**Example**: `handoffs/impl-expo-lx5q8zk3-a7b2.md`
...
1. **Same agent type**: Search `handoffs/` for files starting with your agent prefix
   - Example: `impl-expo-*` for impl-expo agent
...
// 5. Commit changes
git.commit("feat: description [BR-XXX]");
```
